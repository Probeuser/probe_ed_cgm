...
def find_new_data(excel_file_path, db_table_name, type_sebi_text):
    global log_list
    try:
        database_uri = f'mysql://{sebi_config.user}:{sebi_config.password}@{sebi_config.host}/{sebi_config.database}?auth_plugin={sebi_config.auth_plugin}'
        ...
        new_excel_file_path = rf"C:\Users\devadmin\sebi_final_script\ed_cgm\data\incremental_excel_sheets\Missing_Data_{type_sebi_text}_{sebi_config.current_date}.xlsx"
        ...
        traceback.print_exc()
        send_mail.send_email("SEBI ed_cgm order script error", e)
